package mx.com.aprendizaje.arquitectura.ortogonales.reporte.componente;
...
public class StyleComponenteExcel {
	...
	public void generarHojas(Workbook libroExcel, PeticioReporteVO voPeticion) {
		...
		int inicioIndice = 0;
		int finalIndice = sizeDataReport;// por default se toma todo el tama�o del datareport
		...
		for (int indiceHoja = 1; indiceHoja <= noHojas; indiceHoja++) {
			...
			// filasXhoja-1 porque inicia el indice desde 0
			finalIndice = indiceHoja == noHojas ? voPeticion.getContenido().size() - 1 : (filasXhoja * indiceHoja) - 1;
	...
	public synchronized void crearHojaPaginable(Workbook libroExcel, String indexHoja, PeticioReporteVO voPeticion,
		...
		String titHoja = WorkbookUtil.createSafeSheetName((String) (key != null ? key : "Valores Vac�os"));
		Sheet hoja = libroExcel.createSheet(titHoja + " " + indexHoja);
